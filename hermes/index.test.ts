...
describe('Hermes report composition', () => {
  ...
  it('HTML has 3 block headers + recommendation', () => {
    const html = composeWeeklyReportHtml({
      summary: { totalCarbonGrams: 10, totalCostUsd: 0.05, superCallCount: 3, nanoCallCount: 7, totalCalls: 10 },
      top3: [sample],
    });
  ...
  it('Top 3 sorted by cost desc', () => {
    ...
    const html = composeWeeklyReportHtml({
      summary: { totalCarbonGrams: 0, totalCostUsd: 0, superCallCount: 3, nanoCallCount: 0, totalCalls: 3 },
      top3: sortedTop3,
    });
  ...
  it('runWeeklyReport with --dry-run writes HTML file containing Top 3', async () => {
    ...
    try { rmSync(tmpDir, { recursive: true, force: true }); } catch { /* ignore EBUSY on Windows */ }
  });
});
