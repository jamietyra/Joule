...
function parseArgs(argv: string[]): CliArgs {
  ...
  let i = 0;
  if (argv[i] && !argv[i]!.startsWith('--')) { args.subcommand = argv[i]!; i++; }
  if (argv[i] && !argv[i]!.startsWith('--')) { args.reportType = argv[i]!; i++; }
  for (; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--dry-run') args.dryRun = true;
    else if (a === '--db' && argv[i + 1]) { args.db = argv[i + 1]!; i++; }
    else if (a === '--to' && argv[i + 1]) { args.to = argv[i + 1]!; i++; }
    else if (a === '--output' && argv[i + 1]) { args.output = argv[i + 1]!; i++; }
  }
  return args;
}
...
const isMain = import.meta.url === `file://${process.argv[1]?.replace(/\\/g, '/')}` || process.argv[1]?.endsWith('hermes/index.ts') || process.argv[1]?.endsWith('hermes\\index.ts');
if (isMain) {
  ...
  if (args.subcommand === 'run' && args.reportType === 'weekly-report') {
    ...
    console.error('Usage: hermes run weekly-report [--dry-run] [--db <path>] [--to <email>] [--output <path>]');
    process.exit(1);
  }
}
