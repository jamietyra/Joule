...
import { DEFAULT_MODEL_ID, STATIC_TABLE } from "./_static-table"
...
export function measure(
  ...
  if (row === undefined) {
    console.warn(
      `[carbon] unknown modelId "${input.modelId}", falling back to default (${DEFAULT_MODEL_ID})`,
    )
    row = STATIC_TABLE[DEFAULT_MODEL_ID] ?? { grams_per_1k: 0.02, usd_per_1k: 0.001 }
  }
