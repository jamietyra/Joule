import type { ChatRequest, CrusoeInferenceClient, InferenceResult, MockScenario } from "./index.js"
...
export function createMock(scenarios: MockScenario[]): CrusoeInferenceClient {
  return {
    async chat(req: ChatRequest): Promise<InferenceResult> {
      ...
      const matched = scenarios.find((s) => lastUserMessage.content.includes(s.userMessageContains))

      if (matched === undefined) {
        throw new Error(`Mock: no scenario matched message content "${lastUserMessage.content}"`)
      }
