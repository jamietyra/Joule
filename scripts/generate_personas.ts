...
for (let i = 0; i < args.length; i++) {
  if (args[i] === '--db' && args[i + 1]) { dbPath = args[i + 1]; i++; }
  if (args[i] === '--seed' && args[i + 1]) { seed = Number(args[i + 1]); i++; }
}
...
console.log(`Seeded ${totalRecords} records into ${dbPath} (seed=${seed}, personas=${PERSONAS.length}, days=30)`);
